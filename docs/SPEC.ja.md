...
diagram "配信スタジオ 系統図" {
  ...
  order: fixed       # 機器を書いた順に並べる。省略時はレイアウト側が並べ替える
...
`[poe]` は、その Cat ケーブルが**データと一緒に電気も運ぶ**ことを示す。

```khm
sw.LAN1 <-> cam.LAN : lan 30m "N-01" [poe]
```

**別の結線として書いてはならない。** 電源とイーサネットは1本の Cat ケーブルを共有しているので、
2本描けばケーブル表に2行載り、1本しかないものを2本積むことになる。結線に書けば**1つの物、1行**の
ままで、現場で訊かれること — どのポートが PoE でなければならないか、どの機器に電源が要らないか —
に答えられる。

図では番号と長さの隣に `PoE` と出る。表には**給電**の欄が付く。PoE のない図には欄自体が出ない。

Cat ケーブルでない結線に書くと指摘される。同軸に電気は乗らない。`ndi over lan` のように搬送が
Cat であれば書ける — 判定するのは搬送だからである。

値のない属性は**旗印**で、`[poe]` は `[poe=true]` と同じ意味になる。

### 並び順

`order: fixed` を書くと、**機器は書いた順に並ぶ**。省略すると、レイアウトが線の交差を減らすように並べ替える
— ふつうはそちらが望ましいが、ラックのように**順序そのものが情報**であるときは書いた順が正しい。

**図全体の指定である。** group ごとには指定できない。レイアウトエンジンの都合で、配置を尊重する処理は
階層の一部だけに適用できず、一部のグループだけ指定した図は受理されない。**局所的にできないものを
局所的に書けるようにはしない。**
...
`direction` が受けるのは `LR` と `TB` だけで、大文字小文字は問わない。`RL` や `BT` は
黙って逆向きに描かず、指摘される。

`-d/--direction` もテーマと同じ規則で、**ソースが書いていればそちらが勝つ**。group ごとの
向き指定はない。group は外側のレイアウトが置いた機器を囲む枠であって、それ自身がレイアウトを
持つわけではないため。
...
`xlr` だけ大小の語が**前に付く**（`Mini XLR-M`）。末尾は性別のものだからで、読む人も
噛み合わせの判定も、性別を末尾で見つける。
