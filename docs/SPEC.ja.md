...
`xlr` は**オス・メスの組を並べる**。フルサイズ、Mini XLR（TA3）、4極 Mini XLR（TA4）で、
噛み合う相手は**その組の中のもう片方**である。TA3 のプラグに XLR-F が返っては、売っていない
ケーブルを買いに行かせることになる。TA4 は極数が違い、バイアス給電を余分な極で運ぶので
XLR と直結できるものではない。それでも載せる価値はある。「送信機は TA4」は予備のラベリアが
使えるかどうかを決める事実そのもので、書かなければ推測されるからである。

`xlr` だけ大小の語が**前に付く**（`Mini XLR-M`）。末尾は性別のものだからで、`mateOf` も
表を読む人も、性別を末尾で見つける。

型が持たないコネクタは指摘される。`connector` 以外の属性も指摘される。結線の `[…]` は
「追加の属性」としてモデルに保存されるので未知のキーも生き残るが、口の方は保存されないためである。

**殻の大小はコネクタであって型ではない。** 別の型にすれば、カメラをスイッチャーに挿すという
成立する接続に不一致が出る。書く価値は表にある。`HDMI Micro → HDMI` は `HDMI → HDMI` とは
別のケーブルで、持って来たか来なかったかのどちらかである。
...
**変換アダプタは `adapter`。** 殻を変えるだけの小物は口の宣言では表せない。書けばケーブルは
ふつうの HDMI-HDMI に戻り、アダプタは資材表に1個載る。**なくせる部品**だからそこに要る。

```khm
adapter mh "micro-HDMI 変換アダプタ" { in IN : hdmi [connector="HDMI Micro"]  out OUT : hdmi [connector=HDMI] }
```

これは §8 のジャック（`trs` / `trs35`）とは分け方が違う。**荷造りが違うからである。** 3.5mm を
1/4″ に挿すには**変換プラグという部品**が要り、なくすことができる。micro-HDMI を挿すには**端が
micro のケーブル**が1本要るだけで、それはケーブル表に載る。分かれているのはコネクタ欄と資材表の
どちらが答えるかであり、どちらも既にある。

代わりに払うものはある。型は書かなくても効くが、**コネクタは書いて初めて効く。** カメラの口に
1回書けば済むが、書かなければ何も気づかない。
...
設置場所・ラック・系統などのまとまりを表す。図では枠で囲まれる。`group` の中に `group` を
入れられる（下記）。
...
- **1つの端に何本届くかは、ソケットについての問いである。** 1つのソケットに2本の
  ケーブルは挿さらず、それは指摘される。ただし電波にソケットは無いので、5台がぶら下がる
  アクセスポイントも、2本のワイヤレスマイクを受ける1台の受信機も、過剰ではない。
  他と同じく搬送が決める。
